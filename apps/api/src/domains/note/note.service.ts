import { inject, injectable } from 'inversify';
import { FilterQuery, Model, Query } from 'mongoose';

import Note, {
  INote,
  NoteListResponse,
  NoteModel,
} from '@nifty/server-lib/models/note';
import Directory, { IDirectory } from '@nifty/server-lib/models/directory';
import { INoteService } from './interfaces';
import { PaginationParams } from '@/types';

@injectable()
export class NoteService implements INoteService {
  private noteModel: Model<INote>;
  private directoryModel: Model<IDirectory>;

  constructor() {
    this.noteModel = Note;
    this.directoryModel = Directory;
  }

  async findNoteById(id: string): Promise<ReturnType<typeof Note.findById>> {
    return this.noteModel.findById(id);
  }

  async findNotesByIds(
    ids: string[]
  ): Promise<
    Query<
      (INote & Required<{ _id: string }>)[],
      INote & Required<{ _id: string }>,
      {},
      INote
    >
  > {
    return this.noteModel
      .find({
        _id: {
          $in: ids,
        },
        deleted_at: null,
      })
      .sort({ created_at: -1 });
  }

  async updateNoteById(
    id: string,
    data: Partial<INote>
  ): Promise<Query<any, INote & Required<{ _id: string }>, {}, INote>> {
    return this.noteModel.updateOne(
      {
        _id: id,
      },
      {
        $set: data,
      },
      {}
    );
  }

  async paginateNotes(
    condition: FilterQuery<INote>,
    query: PaginationParams
  ): Promise<Partial<NoteListResponse>> {
    return this.noteModel.paginate({
      ...condition,
      ...query,
    });
  }

  async paginateNotesByCollaboratorId(
    collaboratorId: string,
    query: PaginationParams
  ): Promise<Partial<NoteListResponse>> {
    return this.noteModel.paginate({
      collaborators: {
        $elemMatch: {
          $eq: collaboratorId,
        },
      },
      ...query,
    });
  }

  async paginateNotesByDirectoryId(
    directoryId: string,
    query: PaginationParams
  ): Promise<Partial<NoteListResponse>> {
    const directory = await this.directoryModel.findById(directoryId);
    if (!directory) return { data: [], total: 0, page: 0, has_more: false };

    return this.noteModel.paginate({
      _id: {
        $in: directory.notes,
      },
      deleted_at: null,
      ...query,
    });
  }

  async createNote(createdBy: string, data: Partial<INote>): Promise<INote> {
    const doc = {
      ...data,
      created_by: createdBy,
      parent: null,
    };
    const note = await this.noteModel.create(doc);
    return note;
  }

  async findOrCreate(
    key: Partial<INote>,
    data: Partial<INote>
  ): Promise<[INote, boolean]> {
    const note = await this.noteModel.findOne(key);
    if (note) return [note, false];

    return [await this.noteModel.create(data), true];
  }

  async deleteNoteById(
    id: string
  ): Promise<Query<any, INote & Required<{ _id: string }>, {}, INote>> {
    return this.noteModel.updateOne(
      { _id: id },
      { $set: { deleted_at: new Date() } },
      { new: true }
    );
  }

  async findNoteNeighbors(
    noteId: string,
    directoryId: string,
    sortBy: keyof INote,
    limit: number
  ): Promise<{ before: INote[]; after: INote[] }> {
    const note = await this.findNoteById(noteId);
    if (!note) return { before: [], after: [] };

    const query = {
      _id: {
        $ne: noteId,
      },
      directory_id: directoryId,
      deleted_at: null,
    };

    const [before, after] = await Promise.all([
      this.noteModel
        .find({ ...query, [sortBy]: { $lt: note[sortBy] } })
        .sort({ [sortBy]: -1 })
        .limit(limit),
      this.noteModel
        .find({ ...query, [sortBy]: { $gt: note[sortBy] } })
        .sort({ [sortBy]: 1 })
        .limit(limit),
    ]);

    return {
      before,
      after,
    };
  }

  async getKMostRecentNotes(ids: string[], k: number): Promise<INote[]> {
    return this.noteModel
      .find(
        {
          _id: {
            $in: ids,
          },
          deleted_at: null,
        },
        {
          _id: 1,
          title: 1,
          created_at: 1,
          updated_at: 1,
          collaborators: 1,
          public_permissions: 1,
        }
      )
      .sort({ updated_at: -1 })
      .limit(k);
  }
}
