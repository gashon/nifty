...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn('collaborator_id', 'integer', (col) =>
      col.notNull().references('collaborator.id')
    )
    .addColumn('user_id', 'integer', (col) =>
      col.notNull().references('user.id')
    )
    .addUniqueConstraint('note_unique_collaborator', [
      'note_id',
      'collaborator_id',
    ])
    .addColumn('created_at', 'timestamp', (col) =>
  ...
  await db.schema
    .createIndex('note_collaborator_user_id_index')
    .on('note_collaborator')
    .column('user_id')
    .execute();
  ...
  await db.schema
    ...
    .addColumn('deleted_at', 'timestamp')
    .addUniqueConstraint('quiz_unique_collaborator', [
      'quiz_id',
      'collaborator_id',
    ])
    .execute();
  ...
  await db.schema
    .createIndex('quiz_collaborator_user_id_index')
    .on('quiz_collaborator')
    .column('user_id')
    .execute();
  ...
  await db.schema
    ...
    .addColumn('deleted_at', 'timestamp')
    .addUniqueConstraint('directory_unique_collaborator', [
      'directory_id',
      'collaborator_id',
    ])
    .execute();
  ...
  await db.schema
    .createIndex('directory_collaborator_user_id_index')
    .on('directory_collaborator')
    .column('user_id')
    .execute();
}
