...
export type IFreeResponseSubmissionAnswer = Extract<
  ISubmissionAnswer,
  { type: 'free-response' }
>;
export type IMultipleChoiceSubmissionAnswer = Extract<
  ISubmissionAnswer,
  { type: 'multiple-choice' }
>;
export type IFreeResponseSubmissionGradingResponse = Omit<
...
>;
export type IQuizSubmissionAnswer =
...
export type SubmissionModel = mongoose.Model<ISubmission>;
...
export type SubmissionCreateRequest = Pick<ISubmission, 'time_taken'> & {
  answers: IQuizSubmissionAnswer[];
} & { quiz_id: string };
export type SubmissionUpdateRequest = Partial<
  Expand<Omit<ISubmission, 'created_by'>>
>;
